...
- (instancetype)initWithVideoCamera:(GPUImageVideoCamera *)camera withImageSize:(CGSize)size withBitrate:(int) bitrate{
  self = [super initWithImageSize:size resultsInBGRAFormat:YES];
  if (self) {
    _videoCamera = camera;
//    _videoCamera.audioEncodingTarget = (id) self;

    _session = [[VCSimpleSession alloc] initWithVideoSize:size frameRate:_videoCamera.frameRate bitrate:bitrate];
